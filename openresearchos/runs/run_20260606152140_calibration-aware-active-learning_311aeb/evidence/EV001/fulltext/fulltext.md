...
of our method.
2.1
MULTI-SCALE APPROACHES IN DIFFUSION MODELS
...
[CAPTION] Figure 1 illustrates the quality of samples generated by our model across different experimental runs
...
2.2
ADAPTIVE MECHANISMS IN GENERATIVE MODELS
...
[CAPTION] Figure 1: Generated samples from our adaptive dual-scale diffusion model across different runs and
...
3.1
PROBLEM SETTING
We focus on applying diffusion models to 2D datasets. Let X ⊂R2 be our data space, and pdata(x)
...
The diffusion process is defined over T timesteps. Let x0 ∼pdata(x) be a sample from the data
...
w(t) = Softmax(MLPw(t))
(6)
where w(t) ∈R2 represents the weights for the global and local branches at timestep t. The weight
network MLPw is implemented as:
MLPw(t) = Linear2(LeakyReLU(Linear1(SinusoidalEmbedding(t))))
(7)
...
∥ϵ −ϵθ(xt, t)∥2
(9)
where ϵ is the noise added during the forward process, and the expectation is taken over timesteps t,
...
[CAPTION] Figure 2: Evolution of global and local feature weights across timesteps for different datasets. The

[CAPTION] Figure 2 illustrates how the weights for global and local features evolve across timesteps for different
...
[CAPTION] Table 1 summarizes the key performance metrics for each run across the datasets.
...
[CAPTION] Table 1: Performance metrics for different experimental runs across datasets

[CAPTION] Figure 1 provides a visual comparison of the generated samples across different runs and datasets.

[CAPTION] Figure 2 visualizes how the weights for global and local features evolve across timesteps for different
...
models.
2.1
ADAPTIVE NOISE SCHEDULING IN DIFFUSION MODELS
...
2.2
LOW-DIMENSIONAL APPLICATIONS OF DIFFUSION MODELS
...
[CAPTION] Figure 1 showcases the generated samples from our model across different datasets and experimental
...
[CAPTION] Figure 1: Generated samples from our multi-scale grid-based noise adaptation model for circle, dino,
...
AI-Scientist Generated Preprint
2.3
GRID-BASED AND SPATIAL ADAPTATIONS IN GENERATIVE MODELS
...
3.1
PROBLEM SETTING
Let X ⊂Rd be a low-dimensional data space, where d is typically small (e.g., d = 2 in our
...
pθ(xt−1|xt) = N(xt−1; µθ(xt, t, α(xt, t)), Σθ(xt, t, α(xt, t)))
(5)
5
...
[CAPTION] Table 1: Summary of results for different model configurations across all datasets
...
[CAPTION] Figure 2: PLEASE FILL IN CAPTION HERE

[CAPTION] Figure 3 shows the training loss over time for each dataset across all model configurations.
...
[CAPTION] Figure 3: Training loss over time for each dataset (circle, dino, line, and moons) across all runs.

[CAPTION] Figure 1 showcases the generated samples from our full model for each dataset, demonstrating the
...
Ladv = Ex0 [log Dϕ(x0)] + Ext [log(1 −Dϕ(fθ(xt, t)))] .
(2)
...
6.3
RESULTS WITH FINE-TUNED HYPERPARAMETERS
...
[CAPTION] Table 1: Baseline results for the GAN-enhanced diffusion model on four datasets.

[CAPTION] Table 2: Results with gradient penalty for the GAN-enhanced diffusion model on four datasets.

[CAPTION] Table 3: Results with fine-tuned hyperparameters for the GAN-enhanced diffusion model on four
...
AI-Scientist Generated Preprint
6.4
RESULTS WITH QUADRATIC BETA SCHEDULE
...
[CAPTION] Table 4: Results with quadratic beta schedule for the GAN-enhanced diffusion model on four datasets.

[CAPTION] Figure 1 shows the training loss over time for each dataset across different runs. The x-axis represents

[CAPTION] Figure 1: Training loss over time for each dataset across different runs.

[CAPTION] Figure 2 visualizes the generated samples for each dataset across different runs. Each row corresponds
...
[CAPTION] Figure 2: Generated samples from the GAN-enhanced diffusion model for each dataset.
...
we tackle.
2.2
MULTI-EXPERT APPROACHES IN GENERATIVE MODELS
...
2.3
TECHNIQUES FOR IMPROVING MODE CAPTURE
...
[CAPTION] Figure 1: Comparison of KL divergence values across different runs and datasets, demonstrating the
...
ϵθ(xt, t) = gθ(xt, t) · e1(xt, t) + (1 −gθ(xt, t)) · e2(xt, t)
(3)
...
[CAPTION] Figure 2: Generated samples for the ‘dino’ dataset across different runs, showcasing the improved
...
[CAPTION] Table 1 summarizes the key performance metrics for both the baseline model and our dual-expert

[CAPTION] Table 1: Performance comparison between baseline and dual-expert models

[CAPTION] Figure 3 illustrates the training loss curves for the ‘dino’ dataset across different model configurations.

[CAPTION] Figure 4 showcases the generated samples for the ‘dino’ dataset across different model configurations.
...
[CAPTION] Figure 3: Training loss curves for the ‘dino’ dataset, comparing the baseline model with different

[CAPTION] Figure 4: Generated samples for the ‘dino’ dataset, comparing the baseline model with different
...
[CAPTION] Figure 5: Distribution of gating weights for the ‘dino’ dataset, illustrating the specialization of the

[CAPTION] Table 2: Ablation study results for the ‘dino’ dataset
...
We define a set of learnable style embeddings Es ∈RK×D, where K = 4 is the number of predefined
...
p(s|x) = softmax(W2ReLU(W1hL + b1) + b2)
(3)
where hL ∈RH is the last hidden state, H is the hidden dimension of the transformer, W1 ∈RH×H,
W2 ∈RK×H, and b1, b2 are learnable parameters.
4.3
STYLEADAPTER MODULE
...
SAl(hl, s) = hl ⊙(Wls + bl)
(4)
where hl ∈RT ×H is the hidden state at layer l, T is the sequence length, s ∈RD is the style
embedding, Wl ∈RH×D and bl ∈RH are learnable parameters, and ⊙denotes element-wise
multiplication.
4.4
...
h0 = Embed(x) + PosEmbed(x)
(5)
hl = TransformerLayerl(hl−1),
l = 1, . . . , L
(6)
hl = SAl(hl, s),
l = 1, . . . , L
(7)
p(s|x) = StyleClassifier(hL)
(8)
y = LMHead(hL)
(9)
...
The models were trained using the AdamW optimizer with learning rates of 1 × 10−3 for shake-
speare_char and 5 × 10−4 for enwik8 and text8. We employed a cosine learning rate schedule with
...
[CAPTION] Table 2: Performance Comparison: Multi-Style Adapter vs. Baseline

[CAPTION] Table 2 presents a comprehensive comparison between our Multi-Style Adapter and the baseline

[CAPTION] Figure 1: Validation loss curves for enwik8 dataset

[CAPTION] Figure 1 illustrates the validation loss curves for both the baseline and Multi-Style Adapter models on
...
Inference Speed
(tokens/s)
Full Multi-Style Adapter
0.9488 ± 0.0056
1.0000 ± 0.0000
403.99 ± 3.12
Without Style Classification
0.9723 ± 0.0061
0.8912 ± 0.0237
452.31 ± 3.76
StyleAdapter every 2 layers
0.9612 ± 0.0059
0.9567 ± 0.0183
478.65 ± 3.89
...
[CAPTION] Table 3: Ablation Study: Impact of Multi-Style Adapter Components (enwik8 dataset)

[CAPTION] Figure 2: Style consistency scores across datasets and runs

[CAPTION] Figure 2 shows the style consistency scores across different datasets and runs. The high scores,

[CAPTION] Figure 3 compares the inference speed (tokens per second) across different datasets and runs. The

[CAPTION] Figure 4 compares the training time across different datasets and runs. The Multi-Style Adapter

[CAPTION] Figure 5 illustrates the inference time across different datasets and runs. The Multi-Style Adapter
...
[CAPTION] Figure 3: Inference speed comparison across datasets and runs

[CAPTION] Figure 4: Training time comparison across datasets and runs
...
[CAPTION] Figure 5: Inference time comparison across datasets and runs
...
agent aims to learn a policy π(st) that maximizes the expected cumulative reward R = PT
t=0 γtrt,
...
[CAPTION] Table 1: Comparison of baseline and Q-learning methods across different datasets.
...
[CAPTION] Table 2: Ablation study results for different variations of the Q-learning method.

[CAPTION] Figure 1: Training and validation loss for shakespeare_char dataset across different runs.

[CAPTION] Figure 2: Training and validation loss for enwik8 dataset across different runs.
...
els using static or heuristic-based learning rate schedules.
The Q-learning method achieved
...
[CAPTION] Figure 3: Training and validation loss for text8 dataset across different runs.
...
{T1, . . . , Tn}. Each task Ti is defined as an operation over a finite field Fp, where p = 97 (a prime
number). The model receives input sequences x = (x1, . . . , xm), where each xj ∈Fp, and is trained
...
sequences x = (x1, . . . , xm) with xj ∈Fp, and Y = Fp is the output space. The model architecture
...
2:
for each initialization strategy Sj ∈S do
3:
for k = 1 to 3 do
...
6:
Update θt using AdamW and learning rate η(t)
7:
Record Ltrain(θt), Lval(θt), Accval(θt)
8:
end for
9:
Calculate S99 for this run
10:
end for
11:
Compute mean and standard error of metrics across the three runs
12:
end for
13:
Compare performance of different initialization strategies for task Ti
14: end for
...
[CAPTION] Figure 1: Training and Validation Accuracy for x_plus_y task across different initialization methods
...
[CAPTION] Figure 2: Training and Validation Loss for x_minus_y task across different initialization methods

[CAPTION] Figure 3: Training and Validation Accuracy for x_div_y task across different initialization methods

[CAPTION] Figure 4: Training and Validation Loss for permutation task across different initialization methods

[CAPTION] Figure 4 shows that Orthogonal initialization significantly outperformed other methods, achieving the
...
[CAPTION] Table 1: 95% Confidence Intervals for Steps to 99% Validation Accuracy (S99)
...
Layer-wise learning rate methods have gained attention
...
Optimization in Transformer Models:
In the context of Transformer models, Shea & Schmidt
...
Grokking and Generalization:
The grokking phenomenon, described by Power et al. (2022),
...
Algorithmic Learning Tasks:
In the domain of algorithmic learning tasks, most existing work
...
{(xi, yi)}N
i=1, where xi represents an input sequence and yi the corresponding target output. The
...
Tasks and Datasets:
We evaluated our approach on four algorithmic tasks:
...
Model Architecture:
We implemented a Transformer model Vaswani et al. (2017) using PyTorch
...
We used the AdamW optimizer Loshchilov & Hutter (2017) with β1 =
...
and
nn.CrossEntropyLoss.
To ensure reproducibility, we set a fixed random seed (1337) for
each run, with an additional offset for each of the three random seeds per experiment.
Baseline Comparison:
We compared our approach against a baseline uniform learning rate strategy
using a single learning rate of 1 × 10−3 for all model parameters.
Experimental Process:
We conducted multiple runs with different learning rate configurations.
...
[CAPTION] Table 1: Summary of results comparing baseline uniform learning rate approach (Run 0) with our
...
[CAPTION] Figure 1 illustrates the validation accuracy curves for all tasks, comparing the baseline and our

[CAPTION] Figure 1: Validation accuracy curves for all tasks, comparing baseline (Run 0) and layer-wise learning

[CAPTION] Table 2: Ablation study results for the permutation task, comparing our full method against variants
...
{(xi, yi)}{N}
i=1 consists of input-output pairs, where xi represents the input and yi the corresponding
label.
...
log Pθ(yi|xi)
(1)
where Pθ(yi|xi) is the probability assigned by the model to the correct label yi given input xi.
...
We estimate the MDL of a model with parameters θ by pruning weights below a threshold ϵ and
counting the remaining non-zero weights:
MDL(θ) ≈|{wi ∈θ : |wi| > ϵ}|
(3)
where ϵ = 10−2 in our experiments. This computationally efficient approximation allows us to track
...
[CAPTION] Table 1: Final performance metrics across datasets (mean values over 3 runs)

[CAPTION] Table 1 summarizes the final performance metrics. The modular arithmetic tasks achieved near-perfect

[CAPTION] Figure 1 illustrates the grokking phenomenon observed in the x_div_y task. The validation accuracy

[CAPTION] Table 2: Grokking points (steps to reach 95% and 99% validation accuracy)

[CAPTION] Table 2 shows the average number of steps required to reach 95% and 99% validation accuracy. The
...
[CAPTION] Figure 1: Validation accuracy and normalized MDL for x_div_y task

[CAPTION] Figure 2: MDL transition points vs. grokking points across datasets

[CAPTION] Figure 2 compares the MDL transition points (steepest decrease in MDL) with the grokking points

[CAPTION] Figure 3 shows the correlation between MDL reduction and validation accuracy improvement. The

[CAPTION] Figure 4 illustrates the MDL evolution and generalization gap (difference between training and

[CAPTION] Figure 5 compares the MDL transition rate (minimum gradient of MDL) with the grokking speed
...
[CAPTION] Figure 3: Correlation between MDL reduction and validation accuracy improvement

[CAPTION] Figure 4: MDL evolution and generalization gap for x_div_y task
...
[CAPTION] Figure 5: MDL transition rate vs. grokking speed across datasets

[CAPTION] Figure 6: Training metrics for x_div_y task
...
a ◦b →(−a mod p) ◦b or a ◦(−b mod p) or (−a mod p) ◦(−b mod p)
(2)
3
...
pairs of operands (a, b) where a, b ∈Zp for addition and subtraction, and a ∈Zp, b ∈Zp \ {0} for
...
[CAPTION] Figure 1: Validation accuracy over training steps for division operation under different augmentation

[CAPTION] Figure 4 illustrates the validation accuracy curves for the division operation under different augmen-
...
[CAPTION] Figure 2: Validation accuracy over training steps for addition operation under different augmentation

[CAPTION] Figure 2 illustrates the validation accuracy curves for the addition operation. The combined augmen-

[CAPTION] Figure 3: Validation accuracy over training steps for subtraction operation under different augmenta-
...
[CAPTION] Figure 4: Validation accuracy over training steps for division operation under different augmentation

[CAPTION] Figure 4 shows that while all augmentation strategies improved over the baseline, negation augmenta-

[CAPTION] Table 1: Steps to 99% validation accuracy for different operations and augmentation strategies.

[CAPTION] Table 1 highlights the varying effects of augmentation strategies across operations. While combined
...
[CAPTION] Figure 5: Training dynamics for division operation under different augmentation strategies.

[CAPTION] Figure 5 shows the training accuracy and loss curves for the division operation. The sharp increase in
