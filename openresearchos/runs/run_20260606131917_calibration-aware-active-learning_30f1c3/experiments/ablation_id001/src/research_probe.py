...
Topic-aware: picks dataset/model/metric based on spec.topic keyword matching.
...
seeds = [42,1337]
idea_title = spec.get("selected_idea", {}).get("title", "Unknown idea")
...
# ── Topic-aware dataset + model selection ─────────────────────────────────────
def is_topic(*kws): return any(k in topic for k in kws)
...
if is_topic("nlp","text","language","transformer","llm","bert","gpt","rag",
...
try:
    ...
    ax1.set_ylabel("Accuracy"); ax1.set_title(f"{task_type} ({dataset_name})")
    ...
    ax2.plot(seeds,base_accs,"o--",color="#6B7280",label="Baseline",lw=2)
    ax2.plot(seeds,prop_accs,"o-",color="#2563EB",label="Proposed",lw=2)
...
|--------|----------------------|---------------|-------|
"""
table += f"""| Baseline | {bm:.4f} ± {bs:.4f} | {float(np.mean(base_f1s)):.4f} | {seeds} |
"""
table += f"""| **Proposed** | **{pm:.4f} ± {ps:.4f}** | **{float(np.mean(prop_f1s)):.4f}** | {seeds} |
...
metrics = {
    ...
    "proposed_name": "Proposed (topic-tailored)",
    ...
    "success": (bool(better and significant) if spec["level"] in ("probe", "mvp", "ablation") else bool(better)),
